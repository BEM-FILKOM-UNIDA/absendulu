...
export default function ProfileEditor({ email, fullName, nim, division, userType, canEditDivision }: ProfileEditorProps) {
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      const body = canEditDivision ? form : { full_name: form.full_name, nim: form.nim }
      ...
      setForm((current) => ({ ...current, full_name: result.full_name, nim: result.nim, division: result.division || '' }))
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardTitle className="mt-2 text-xl">Profil {userType === 'mahasiswa' ? 'mahasiswa' : 'pengguna'}</CardTitle>
        <CardDescription>Nama dan {identifierLabel} dapat diperbarui. Email Google dan Fakultas ditetapkan oleh sistem.</CardDescription>
      </CardHeader>
      <CardContent>
        <form onSubmit={handleSubmit} className="space-y-5">
          <div className="grid gap-5 sm:grid-cols-2">
            ...
            <div className="space-y-2">
              <Label htmlFor="profile-identifier">{identifierLabel}</Label>
              <Input id="profile-identifier" value={form.nim} onChange={(event) => updateField('nim', event.target.value.toUpperCase())} autoComplete="off" placeholder={userType === 'mahasiswa' ? 'I.2410036' : 'Nomor identitas'} required maxLength={64} />
              {userType === 'mahasiswa' && <p className="text-xs leading-5 text-[var(--muted)]">Format wajib: I. diikuti 7 angka.</p>}
            </div>
            <div className="space-y-2">
            ...
            {canEditDivision && <div className="space-y-2"><Label htmlFor="profile-division">Divisi <span className="font-normal text-[var(--muted)]">(opsional)</span></Label><Input id="profile-division" value={form.division} onChange={(event) => updateField('division', event.target.value)} maxLength={100} /></div>}
            <div className="space-y-2">
