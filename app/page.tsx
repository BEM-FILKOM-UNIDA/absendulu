...
export default function LandingPage() {
    ...
    <main className="paper-noise overflow-hidden bg-[var(--paper)]">
      <nav className="mx-auto flex h-20 max-w-7xl items-center justify-between px-4 sm:h-24 sm:px-5 lg:px-8">
        ...
        <div className="flex items-center gap-3 sm:gap-5">
          <span className="hidden text-xs font-bold uppercase tracking-[.14em] text-[var(--muted)] sm:inline">absen cepat, satu scan</span>
      ...
      <section className="mx-auto grid max-w-7xl gap-10 px-4 pb-14 pt-6 sm:gap-12 sm:px-5 sm:pb-20 sm:pt-10 lg:grid-cols-[.9fr_1.1fr] lg:items-center lg:gap-20 lg:px-8 lg:pb-28 lg:pt-16">
        <div className="animate-rise-in">
          <p className="eyebrow mb-5 flex max-w-full items-center gap-3 whitespace-nowrap text-[.58rem] tracking-[.11em] text-[var(--accent-strong)] sm:mb-7 sm:text-[.68rem] sm:tracking-[.18em]"><span className="h-2 w-2 shrink-0 bg-[var(--accent)]" />Scan. Check-in. Done.</p>
  ...
  Absen <br className="hidden sm:block" /><em className="text-[var(--accent-strong)]">Dulu.</em>
          ...
          <div className="mt-7 flex flex-wrap items-center gap-4 sm:mt-9">
            <ButtonLink href="/login" variant="accent" className="w-full sm:w-auto">Mulai absensi <span aria-hidden="true">↗</span></ButtonLink>
        ...
        <div className="animate-rise-in-delay relative mt-2 sm:mt-0">
          ...
          <Card className="live-demo-card relative overflow-hidden border-[var(--ink)] p-4 shadow-[10px_12px_0_var(--accent)] sm:p-8 sm:shadow-[18px_22px_0_var(--accent)]" style={{ backgroundColor: 'var(--ink)', color: '#f7f4ed' }}>
            <div className="flex items-start justify-between gap-3 border-b border-white/15 pb-5 sm:pb-6">
              <div><p className="eyebrow text-[var(--accent)]">absensi live</p><h2 className="mt-2 text-xl font-black tracking-[-.05em] sm:mt-3 sm:text-2xl">Seminar Karier Digital</h2></div>
            ...
            <div className="grid gap-6 pt-6 sm:grid-cols-[.85fr_1.15fr] sm:items-center sm:gap-8 sm:pt-8">
              <div>
                ...
                <p className="mt-3 text-xs font-bold text-[var(--accent)]">75% hadir sekarang</p>
              </div>
  ...
  <div className="mx-auto max-w-7xl">
    <div className="flex flex-col justify-between gap-5 md:flex-row md:items-end">
      ...
      <p className="max-w-xs text-base leading-7 text-[var(--muted)] sm:text-lg">Tiga langkah, kurang dari 10 detik, tanpa antre dan tanpa kertas.</p>
    </div>
    <div className="mt-10 grid border-t border-[var(--border)] sm:mt-14 md:grid-cols-3">
      ...
      <footer className="mx-auto flex max-w-7xl flex-col gap-3 px-4 py-8 text-xs font-semibold text-[var(--muted)] sm:flex-row sm:items-center sm:justify-between sm:px-5 lg:px-8"><p>© 2025 Absendulu. Hadir tanpa kertas.</p><p>Absensi yang nggak bikin antre.</p></footer>
    </main>
  )
}
