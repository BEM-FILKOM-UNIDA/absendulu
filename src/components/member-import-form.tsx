...
export function MemberImportForm() {
  ...
  return (
    <div className="grid gap-6 lg:grid-cols-2">
      <Card className="border-2 border-(--ink) shadow-[7px_8px_0_var(--ink)]">
        <div className="p-6 pb-4">
          <p className="eyebrow text-(--accent-strong)">daftarkan banyak orang</p>
          <h2 className="mt-2 text-2xl font-black text-(--ink)">Import data FILKOM</h2>
          <p className="mt-2 text-sm leading-6 text-(--muted)">Unggah data mahasiswa tanpa mengirim email massal.</p>
        </div>
        <div className="p-6 pt-2">
          <div className="mb-5 border-l-4 border-(--accent-strong) bg-(--surface-muted) px-4 py-3 text-sm leading-6">
            <p className="mb-1 font-bold text-(--ink)">Format kolom CSV</p>
            <code className="break-all font-bold text-(--accent-strong)">full_name,nim,email,user_type,division,phone</code>
            <p className="mt-1 text-(--muted)">Tipe pengguna: mahasiswa, dosen, atau tata_usaha.</p>
          </div>
      ...
      <Card className="border-(--accent-strong) shadow-[7px_8px_0_var(--accent)]">
        <div className="p-6 pb-4">
          <p className="eyebrow text-(--accent-strong)">daftarkan satu orang</p>
          <h2 className="mt-2 text-xl font-black">Aktivasi manual</h2>
          <p className="mt-2 text-sm leading-6 text-(--muted)">Akun langsung aktif untuk login Google atau Magic Link.</p>
        </div>
        <div className="p-6 pt-2">
          <form onSubmit={handleManualCreate} className="grid gap-4 sm:grid-cols-2">
            <label className="space-y-2 sm:col-span-2"><span className="text-sm font-bold">Nama lengkap</span><input value={manualForm.full_name} onChange={(event) => updateManualField('full_name', event.target.value)} placeholder="Nama mahasiswa" className="h-12 w-full border border-(--border) bg-(--surface-strong) px-4 text-sm" required /></label>
            <label className="space-y-2"><span className="text-sm font-bold">NIM / NIP</span><input value={manualForm.nim} onChange={(event) => updateManualField('nim', event.target.value.toUpperCase())} placeholder="Contoh: I.2410036" className="h-12 w-full border border-(--border) bg-(--surface-strong) px-4 text-sm" required /></label>
            <label className="space-y-2"><span className="text-sm font-bold">Tipe pengguna</span><select value={manualForm.user_type} onChange={(event) => updateManualField('user_type', event.target.value)} className="h-12 w-full border border-(--border) bg-(--surface-strong) px-4 text-sm"><option value="mahasiswa">Mahasiswa</option><option value="dosen">Dosen</option><option value="tata_usaha">Tata Usaha</option></select></label>
            ...
            <label className="space-y-2"><span className="text-sm font-bold">Divisi <span className="font-normal text-(--muted)">(opsional)</span></span><input value={manualForm.division} onChange={(event) => updateManualField('division', event.target.value)} placeholder="Contoh: EKAKARSA" className="h-12 w-full border border-(--border) bg-(--surface-strong) px-4 text-sm" /></label>
