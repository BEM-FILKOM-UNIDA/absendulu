...
function HistoryPage() {
  ...
  return (
    <div className="space-y-8">
      <section className="border-b border-(--border) pb-8"><p className="eyebrow text-(--accent-strong)">{eyebrow}</p><h1 className="display-type mt-3 text-4xl leading-none tracking-[-.07em] sm:text-5xl">Jejak<br /><em>kehadiranmu.</em></h1><p className="mt-4 max-w-md text-sm leading-6 text-(--muted)">{description}</p></section>
      <Card className="overflow-hidden"><div className="border-b border-(--border) px-5 py-5"><p className="eyebrow text-(--accent-strong)">{heading}</p><h2 className="mt-2 text-lg font-black">Riwayat terbaru</h2></div>{attendances.length > 0 ? <div className="divide-y divide-(--border)">{attendances.map((attendance) => <HistoryRow key={attendance.id} attendance={attendance} isAdmin={isAdmin} />)}</div> : <div className="px-6 py-16 text-center text-sm text-(--muted)">Belum ada riwayat absensi.</div>}</Card>
    </div>
  )
}
...
function HistoryRow({ attendance, isAdmin }: { attendance: HistoryItem; isAdmin: boolean }) {
  ...
  return <div className="flex flex-col justify-between gap-3 px-5 py-5 sm:flex-row sm:items-center"><div><p className="font-black">{eventName}</p>{isAdmin ? <p className="mt-1 text-xs font-bold text-(--muted)">{participant} · {nim}</p> : null}<p className="mt-2 text-xs text-(--muted)">{checkIn} · {attendance.method}</p>{attendance.notes ? <p className="mt-2 text-xs text-(--muted)">Catatan: {attendance.notes}</p> : null}</div><Badge variant={variant}>{attendance.status}</Badge></div>
}
