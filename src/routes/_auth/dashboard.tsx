  ...
  return (
    <section className="grid border-y border-(--border) sm:grid-cols-2 xl:grid-cols-4">
      {stats.map((stat, index) => {
        ...
        return <div key={stat.label} className={`border-b border-(--border) px-5 py-6 sm:border-r xl:border-b-0 ${index === 0 ? 'bg-(--accent-soft)' : ''}`}>
          <div className="flex items-center justify-between">
            <p className="eyebrow text-(--muted)">{stat.label}</p>
            <span className="font-mono text-xs font-bold text-(--accent-strong)">{number}</span>
          </div>
          <p className="mt-7 text-5xl font-black -tracking-widest tabular-nums">{stat.value}</p>
