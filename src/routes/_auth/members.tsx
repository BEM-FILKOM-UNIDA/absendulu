    ...
    <div className="space-y-8">
      ...
      <Card className="overflow-hidden">
        <div className="flex items-center justify-between border-b border-(--border) px-5 py-5">
          <div><p className="eyebrow text-(--accent-strong)">data anggota</p><h2 className="mt-2 text-lg font-black">Akun terdaftar</h2></div>
          <span className="font-mono text-xs text-(--muted)">{rows.length} akun</span>
        </div>
        {error ? <p role="alert" className="m-5 border border-[#e7b6b6] bg-[#f8dddd] px-4 py-3 text-sm font-semibold text-(--danger)">{error}</p> : null}
        {rows.length > 0 ? (
          <div className="overflow-x-auto">
            <table className="w-full min-w-190 text-sm">
              <thead className="bg-(--surface-muted) text-left text-[10px] font-black uppercase tracking-widest text-(--muted)"><tr><th className="px-5 py-3">Nama</th><th className="px-5 py-3">NIM/NIP</th><th className="px-5 py-3">Tipe</th><th className="px-5 py-3">Email</th><th className="px-5 py-3">Status</th><th className="px-5 py-3">Tindakan</th></tr></thead>
                ...
                {rows.map((member) => {
                  ...
                  return (
                    <tr key={member.id}>
                      ...
                      <td className="px-5 py-4 text-(--muted)">{member.nim || '-'}</td>
