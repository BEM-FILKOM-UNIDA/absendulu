...
type StudentData = Awaited<ReturnType<typeof getStudentHomeData>>
type StudentEvent = StudentData['events'][number]
type AttendanceItem = StudentData['attendance'][number]

export const Route = createFileRoute('/_auth/mahasiswa')({
  loader: () => ({ data: defer(getStudentHomeData()) }),
...
function StudentHomePage() {
  ...
  return (
    <Suspense fallback={<StudentPending />}>
      <Await promise={data} fallback={<StudentPending />}>
        {(resolved: StudentData) => <StudentHomeContent data={resolved} />}
      </Await>
    </Suspense>
  )
}

function StudentHomeContent({ data }: { data: StudentData }) {
  ...
  return (
    <div className="space-y-8">
      <section className="flex flex-col justify-between gap-6 border-b border-(--border) pb-8 sm:flex-row sm:items-end">
        <div>
          <p className="eyebrow text-(--accent-strong)">workspace mahasiswa / FILKOM</p>
      ...
      <StudentHistory attendance={data.attendance} />
    </div>
  )
}
    ...
    <Card className="overflow-hidden">
      ...
      {events.length > 0 ? (
        <div className="divide-y divide-(--border)">
          {events.slice(0, 5).map((event) => <StudentEventRow key={event.id} event={event} isOpen={open.has(event.id)} />)}
        </div>
      ) : (
        <div className="px-6 py-14 text-center text-sm text-(--muted)">Belum ada acara mendatang.</div>
      )}
    </Card>
  )
}
...
function StudentEventRow({ event, isOpen }: { event: StudentEvent; isOpen: boolean }) {
  ...
  return (
    <Link to="/events" className="flex items-center justify-between gap-4 px-6 py-5 hover:bg-(--surface-muted)">
...
function StudentProfile({ email, nim }: { email: string | null | undefined; nim: string | null | undefined }) {
  ...
  return (
    <Card className="bg-(--ink) p-7 text-[#f7f4ed]">
      <p className="eyebrow text-(--accent)">profil akun</p>
...
function StudentHistory({ attendance }: { attendance: AttendanceItem[] }) {
  return (
    <Card className="overflow-hidden">
      ...
      {attendance.length > 0 ? (
        <div className="divide-y divide-(--border)">
          {attendance.map((item) => <AttendanceRow key={item.id} item={item} />)}
        </div>
      ) : (
        <div className="px-6 py-12 text-sm text-(--muted)">Belum ada riwayat absensi.</div>
      )}
    </Card>
  )
}
  ...
  return (
    <div className="flex flex-col justify-between gap-2 px-6 py-4 sm:flex-row sm:items-center">
      <div>
        ...
        <p className="mt-1 text-xs text-(--muted)">{checkIn} · {item.method}</p>
      </div>
      <Badge variant={item.status === 'hadir' ? 'success' : item.status === 'alpha' ? 'danger' : 'muted'}>{item.status}</Badge>
    </div>
  )
}
...
function StudentPending() {
  return (
    <div className="space-y-8" aria-label="Memuat beranda" role="status">
      <div className="space-y-4 border-b border-(--border) pb-8">
        <div className="h-3 w-40 animate-pulse bg-(--surface-muted)" />
        <div className="h-14 w-56 animate-pulse bg-(--surface-muted)" />
