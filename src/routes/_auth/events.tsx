...
function EventsPage() {
  ...
  return (
    <div className="space-y-8">
      ...
      {events.length > 0 ? <div className="grid gap-5 md:grid-cols-2 xl:grid-cols-3">{events.map((event) => <EventCard key={event.id} event={event} />)}</div> : <div className="border border-dashed border-[var(--border)] bg-[var(--surface)] px-6 py-20 text-center"><p className="eyebrow text-[var(--accent-strong)]">belum ada agenda</p><h2 className="display-type mt-4 text-3xl">Belum ada acara.</h2><p className="mx-auto mt-3 max-w-sm text-sm leading-6 text-[var(--muted)]">Acara yang sudah dipublikasikan akan muncul di sini.</p></div>}
    </div>
  )
}
...
function EventCard({ event }: { event: EventItem }) {
  ...
  return (
    <Card className="flex flex-col overflow-hidden">
      <div className="flex items-start justify-between gap-4 border-b border-[var(--border)] p-6"><div><p className="eyebrow text-[var(--accent-strong)]">{event.event_date}</p><h2 className="mt-3 text-xl font-black tracking-[-.04em]">{event.name}</h2></div><Badge variant={variant}>{event.status}</Badge></div>
      <div className="flex flex-1 flex-col p-6"><p className="text-sm leading-6 text-[var(--muted)]">{eventDescription}</p><div className="mt-6 space-y-2 text-xs font-bold text-[var(--muted)]"><p>Waktu: {time}</p><p>Lokasi: {location}</p></div><Link to="/events/$id" params={{ id: event.id }} className="mt-7 text-xs font-black uppercase tracking-[.1em] text-[var(--accent-strong)] hover:underline">Lihat detail ↗</Link></div>
    </Card>
  )
}
