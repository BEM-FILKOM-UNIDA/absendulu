...
function SuccessState({ result }: { result: ScanResult }) { return <Card className="border-(--accent-strong) bg-(--accent-soft) p-8 text-center"><div className="mx-auto grid h-16 w-16 place-items-center bg-(--ink) text-(--lime)"><Check aria-hidden="true" className="h-8 w-8" /></div><p className="eyebrow mt-6 text-(--accent-strong)">absensi berhasil</p><h2 className="mt-2 text-2xl font-black">Kehadiran tercatat.</h2><p className="mt-2 text-sm text-(--muted)">{result.eventName} · {result.status}</p><ButtonLink href="/scan" variant="primary" className="mt-7">Scan lagi</ButtonLink></Card> }

function ScannerCard({ result, loading, onScan }: { result: ScanResult | null; loading: boolean; onScan: (token: string) => void }) { return <Card className="overflow-hidden p-5 sm:p-8"><div className="mb-6 flex items-center justify-between border-b border-(--border) pb-4"><p className="eyebrow text-(--muted-soft)">kamera siap digunakan</p><span className="inline-flex items-center gap-1.5 text-[10px] font-black uppercase tracking-widest text-(--accent-strong)"><span aria-hidden="true" className="h-1.5 w-1.5 rounded-full bg-current" />siap scan</span></div>{result?.error ? <div role="alert" className="mb-5 border border-[#e7b6b6] bg-[#f8dddd] px-4 py-3 text-sm font-semibold text-(--danger)">{result.error}</div> : null}{loading ? <div className="grid min-h-64 place-items-center text-center"><div><span className="mx-auto mb-4 block h-8 w-8 animate-spin rounded-full border-2 border-(--border) border-t-(--accent-strong)" /><p className="text-sm font-bold">Mencatat kehadiran…</p></div></div> : <Suspense fallback={<div className="grid min-h-64 place-items-center text-sm text-(--muted)">Memuat kamera…</div>}><QRScanner onScan={onScan} /></Suspense>}</Card> }
