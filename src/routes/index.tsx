  ...
  return (
    <div className="grid aspect-square w-full grid-cols-7 gap-1.5 sm:gap-2" aria-label="QR signal grid" role="img">
      {Array.from({ length: 49 }, (_, index) => (
        <span key={index} className={`aspect-square ${signalCells.has(index) || (index * 11 + index * index) % 9 < 2 ? 'bg-(--ink)' : 'bg-transparent'}`} />
      ))}
    </div>
  )
}
...
function LandingPage() {
  ...
  return (
    <main className="paper-noise overflow-hidden bg-(--paper)">
      <nav className="mx-auto flex h-20 max-w-7xl items-center justify-between px-4 sm:h-24 sm:px-5 lg:px-8">
        <Link to="/" className="flex items-center gap-3">
          ...
          <span className="text-[10px] font-bold uppercase tracking-[.18em] text-(--muted) sm:text-xs">FILKOM UNIDA</span>
        </Link>
      ...
      <section className="mx-auto grid max-w-7xl gap-10 px-4 pb-14 pt-6 sm:gap-12 sm:px-5 sm:pb-20 sm:pt-10 lg:grid-cols-[.9fr_1.1fr] lg:items-center lg:gap-20 lg:px-8 lg:pb-28 lg:pt-16">
        <div className="animate-rise-in">
          <p className="eyebrow mb-5 flex items-center gap-3 text-(--accent-strong) sm:mb-7"><span className="h-2 w-2 shrink-0 bg-(--accent)" />HADIR DALAM SATU SCAN</p>
        ...
        <div className="animate-rise-in-delay relative">
          <div className="absolute -right-8 -top-8 h-28 w-28 rounded-full bg-(--lime) blur-3xl sm:h-36 sm:w-36" />
          <div className="relative overflow-hidden border-(--ink) bg-(--ink) p-4 text-[#f7f4ed] shadow-[10px_12px_0_var(--accent)] sm:p-8 sm:shadow-[18px_22px_0_var(--accent)]">
            <div className="flex items-start justify-between gap-3 border-b border-white/15 pb-5 sm:pb-6"><div><p className="eyebrow text-(--accent)">ACARA SEDANG BERLANGSUNG</p><h2 className="mt-2 text-xl font-black tracking-[-.05em] sm:text-2xl">Seminar Karier Digital</h2></div><span className="flex shrink-0 items-center gap-2 text-xs font-bold text-(--lime)"><span className="signal-pulse h-2 w-2 rounded-full bg-(--lime)" /> SEDANG DIBUKA</span></div>
            <div className="grid gap-6 pt-6 sm:grid-cols-[.85fr_1.15fr] sm:items-center sm:gap-8 sm:pt-8"><div><p className="eyebrow text-white/45">sudah hadir</p><p className="mt-2 text-6xl font-black -tracking-widest text-(--lime) sm:text-7xl">24</p><p className="mt-2 text-sm text-white/55">dari 32 peserta terdaftar</p><div className="mt-6 h-1.5 bg-white/10"><div className="h-full w-3/4 bg-(--accent)" /></div><p className="mt-3 text-xs font-bold text-(--accent)">75% peserta sudah hadir</p></div><div className="relative mx-auto w-full max-w-55 bg-(--paper) p-5 text-(--ink) sm:mx-0 sm:max-w-none sm:p-7"><SignalGrid /><div className="absolute -bottom-3 left-1/2 -translate-x-1/2 whitespace-nowrap bg-(--lime) px-3 py-1 text-[10px] font-black uppercase tracking-[.12em]">scan untuk hadir</div></div></div>
          </div>
        </div>
      </section>

      <section className="border-y border-(--border) bg-(--surface) px-4 py-14 sm:px-5 sm:py-20 lg:px-8 lg:py-28">
        <div className="mx-auto max-w-7xl">
          <div className="flex flex-col justify-between gap-5 md:flex-row md:items-end">
            <div>
              <p className="eyebrow text-(--accent-strong)">cara kerja</p>
            ...
            <p className="max-w-xs text-base leading-7 text-(--muted) sm:text-lg">Absensi selesai dalam hitungan detik. Cukup scan QR yang tersedia di lokasi acara.</p>
          </div>
          <div className="mt-10 grid border-t border-(--border) sm:mt-14 md:grid-cols-3">
            {steps.map(([number, title, description]) => (
              <div key={number} className="border-b border-(--border) py-6 md:border-b-0 md:border-r md:px-7">
                <span className="text-sm font-black text-(--accent-strong)">{number}</span>
                <h3 className="mt-8 text-xl font-black tracking-[-.03em] sm:mt-12 sm:text-2xl">{title}</h3>
                <p className="mt-3 max-w-xs text-base leading-7 text-(--muted)">{description}</p>
              </div>
            ))}
          </div>
        </div>
