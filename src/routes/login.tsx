    ...
    <main className="paper-noise grid min-h-dvh place-items-center overflow-hidden bg-(--paper) px-5 py-10">
      ...
      <div className="relative w-full max-w-md">
        ...
        <div className="overflow-hidden border border-(--ink) bg-(--surface) shadow-[8px_10px_0_var(--accent)]">
          <header className="bg-(--ink) p-7 text-[#f7f4ed] sm:p-8"><p className="eyebrow text-(--accent)">FILKOM UNIDA</p><h1 className="display-type pt-3 text-4xl leading-none tracking-[-.06em]">Masuk ke<br /><em>AbsenDulu.</em></h1><p className="pt-4 text-sm text-white/55">Pilih Google atau email untuk masuk.</p></header>
          <div className="p-7 sm:p-8">
            ...
            <div className="my-6 flex items-center gap-3 text-[10px] font-black uppercase tracking-[.14em] text-(--muted-soft)"><span className="h-px flex-1 bg-(--border)" />atau email<span className="h-px flex-1 bg-(--border)" /></div>
            <form onSubmit={handleLogin} className="space-y-5">
              ...
              <button type="submit" disabled={loading} className="inline-flex min-h-11 w-full items-center justify-center gap-2 rounded-sm bg-(--accent) px-5 text-sm font-bold text-(--accent-foreground) hover:bg-[#55ded4] disabled:opacity-50">{loading ? 'Mengirim…' : 'Kirim Link'} <span aria-hidden="true">↗</span></button>
            </form>
            <p className="mt-7 border-t border-(--border) pt-5 text-center text-xs leading-5 text-(--muted)">Gunakan email yang sudah didaftarkan panitia.</p>
          </div>
        </div>
